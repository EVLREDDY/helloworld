...
sap.ui.controller("metadata.view.S1", {
	 ...
	 onInit : function() {
       ...
       onSearch:function(e){
    	 ...
    	 var password = this.byId("passwordId").getValue();
    	 
    	 if(service == "" && e.oSource.sId.indexOf("reload")== -1){
    		 sap.m.MessageToast.show("Please Enter the oData Service");
    		 return;
    	 }else if(user == ""){
    		 sap.m.MessageToast.show("Please Enter the User Name for the service");
    		 return;
    	 }else if(password == ""){
    		 sap.m.MessageToast.show("Please Enter Password for the service");
    		 return;
    	 }
    	...
    	// metadata.comp._initODataModel(url);
    	 this.oDataModel =  new  sap.ui.model.odata.ODataModel(url,{user:user,password:password})
         setTimeout(function(){
        	debugger;
   	...
   	getProperties:function(data,i){
   		...
   		if(nvaData.length>0){
   			for(var m=0;m<nvaData.length;m++){
   				arr.push({name:nvaData[m].name,children:$.extend(true,[],nvaData[m].children)});
   			} 
   		}
   		return arr;
   	},
